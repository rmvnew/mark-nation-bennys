...
vSERVER = Tunnel.getInterface("nation_bennys")
...
-- 						if vSERVER.checkTuningVehicle() then
...
-- 						else
-- 							TriggerEvent("Notify","negado","Este veiculo não se encontra na garagem de nenhum jogador",5)
-- 						end
-- 					end
-- 				end
-- 			end
...
--         elseif not nui then
--             idle = 5
--             DrawMarker(27, bennys.coords.x, bennys.coords.y, bennys.coords.z - 0.2, 0, 0, 0, 0, 0, 0, 3.0, 3.0, 1.0, 255, 102, 0, 200, 0, 0, 0, 1)
            
--             local playercoords = GetEntityCoords(PlayerPedId())
--             local distance = #(playercoords - bennys.coords)
--             if distance < 2 then
--                 drawTxt("PRESSIONE  ~r~E~w~  PARA ACESSAR A ~y~MECÂNICA", 4, 0.5, 0.93, 0.50, 255, 255, 255, 180)
--                 if IsControlJustPressed(0, 38) then
--                     -- Envia o tipo da mecânica para o servidor para verificar a permissão
--                     if vSERVER.checkPermission(bennys.mecanica) then
--                         vehicle = getNearestVehicle(7)
--                         if vehicle and vSERVER.checkVehicle(VehToNet(vehicle)) then
--                             if vSERVER.checkTuningVehicle() then
--                                 damage = (1000 - GetVehicleBodyHealth(vehicle)) / 100
...
--                                 isVehicleTooFar(vehicle)
--                             else
--                                 TriggerEvent("Notify", "negado", "Este veículo não se encontra na garagem de nenhum jogador", 5)
--                             end
--                         end
--                     else
--                         TriggerEvent("Notify", "negado", "Você não tem permissão para usar esta mecânica.", 5)
--                     end
--                 end
--             end
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        elseif not nui then
            idle = 5
            -- Ajusta o marcador para ficar mais próximo ao chão
            DrawMarker(
                27, 
                bennys.coords.x, 
                bennys.coords.y, 
                bennys.coords.z - 0.5,  -- Ajuste fino para o nível do chão
                0, 0, 0, 
                0, 0, 0, 
                3.0, 3.0, 0.2,  -- Altura reduzida para parecer próximo ao chão
                255, 102, 0, 200, 
                0, 0, 0, 1
            )
...
RegisterCommand("tow",function(source,args)
    ...
    local vehicletow = IsVehicleModel(vehicle,GetHashKey("lotusscania"))
        ...
        if DoesEntityExist(vehicle) and DoesEntityExist(rebocado) then
            if reboque == nil then
                if vehicle ~= rebocado then
                    ...
                    AttachEntityToEntity(rebocado,vehicle,GetEntityBoneIndexByName(vehicle,"bodyshell"),0,-2.2,0.3-min.z,0,0,0,1,1,0,1,0,1)
                    reboque = rebocado
                end
            else
                AttachEntityToEntity(reboque,vehicle,0,0.0,-10.0,1.3,0.0,0.0,0.0,false,false,true,false,5,true)
...
RegisterNUICallback("pagar",function(data)
	if cart["total"] then
		if vSERVER.checkPayment(parseInt(cart["total"])) then
			SetNuiFocus(false,false)
			...
			vSERVER.saveVehicle(mName,mPlaca,myveh)
			src.closeNui()
		end
	end
end)
...
RegisterNUICallback("callbacks",function(data)
	if data then
		if data.type == "reparar" then
			...
			block = true
			if vSERVER.repairVehicle(vehicle,damage) then
				...
				myveh.damage = 0.0
				vRP.stopAnim(false)
				TriggerEvent("Notify","sucesso","Veículo reparado com <b>sucesso</b>.", 5)
...
function isVehicleTooFar(veh)
	Citizen.CreateThread(function()
		while nui do
			...
			if distance > 7 then
				src.closeNui()
				TriggerEvent("Notify","importante","Você se afastou muito do veículo.", 5)
			end
			Citizen.Wait(500)
		end
	end)
...
function getNearestBennys()
    local locais = config.locais
    local playercoords = GetEntityCoords(PlayerPedId())
    for i, localInfo in ipairs(locais) do
        local distance = #(playercoords - localInfo.coords)
        if distance < 3 then
            return localInfo -- Retorna o local e o tipo de mecânica
        end
    end
    return false
end
